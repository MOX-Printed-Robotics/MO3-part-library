cube_width = 20;
connection_depth = 3;
slop = 0.35; // Clearance addendum to all diameters in connection shafts
...
module thruholes() {
  ...
  for (i = [45, 90, 135]) {
    rotate(i, [0, 1, 0])
    cylinder(d = thruhole_diameter, h = cube_width * 2, center = true, $fn = cylinder_faces);
  }
  for (i = [45, 135]) {
    rotate(i, [0, 0, 1])
    rotate(90, [1, 0, 0])
...
module face_connector() {
  difference() {
    ...
    //cutting thruhole
    cylinder(d = thruhole_diameter, h = cube_width * 3, center = true, $fn = cylinder_faces);
